...
- [x] Reduced Chrome cleanup warning latency: `scripts/cleanup_chrome_test_tabs.py` now retries two short 5-second JXA attempts and reports per-attempt metadata instead of waiting once for 15 seconds. Live cleanup completed in about `0.16s`; full `tests.test_safety` passed `989/989`.
